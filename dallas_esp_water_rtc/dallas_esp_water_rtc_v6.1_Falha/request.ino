...
void coletaDados(){
  ...
  noSensor.concat(F("\\\"temperatura\\\":")); noSensor.concat(mediaTemperatura); noSensor.concat(",");
  noSensor.concat(F("\\\"time\\\":\\\"")); noSensor.concat(getDateTime());noSensor.concat("\\\",");
  ////Tratamento de mensagem
  noSensor.concat(F("\\\"tipoAlerta1\\\":"));noSensor.concat(TEMPERATURA);noSensor.concat(",");
  noSensor.concat(F("\\\"mensagem1\\\":\\\""));noSensor.concat(FEBRE);noSensor.concat("\\\",");
  
  noSensor.concat(F("\\\"tipoAlerta2\\\":"));noSensor.concat(UMIDADE);noSensor.concat(",");
  noSensor.concat(F("\\\"mensagem2\\\":\\\""));noSensor.concat(URINA);noSensor.concat("\\\",");
  
  noSensor.concat(F("\\\"tipoAlerta3\\\":"));noSensor.concat(NOALERTA);noSensor.concat(",");
  noSensor.concat(F("\\\"mensagem3\\\":\\\""));noSensor.concat(F(""));noSensor.concat("\\\",");
  
  noSensor.concat(F("\\\"umidade:\\\":")); noSensor.concat(mediaUmidade);noSensor.concat("}\"");
  noSensor.concat(F(" HTTP/1.1\r\n"));
  noSensor.concat(F(" Content-Type: charset=utf-8\r\n"));
 
 //  String HOST_PORT   "80";
  String HOST_NAME= F("192.168.0.15");
